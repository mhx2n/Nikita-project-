    ...
    try:
        ...
        if update.effective_message:
            ...
            logger.info(
                ...
                f"chat_type={msg.chat.type if msg.chat else 'N/A'} "
                f"chat_id={msg.chat.id if msg.chat else 'N/A'} "
...
async def handle_all_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # effective_message — Guest mode সহ সব ধরনের message ধরে
    msg = update.effective_message
    ...
    # 1. Entity mention — Guest mode-এও কাজ করে
    ...
    if not should_reply and msg.reply_to_message:
        ...
        if rt_user:
            if (rt_user.username or "").lower() == bot_username or rt_user.id == context.bot.id:
                should_reply = True
...
# ─── Main ─────────────────────────────────────────────────────────────────────
def main():
    # Flask — আলাদা thread এ, asyncio এর সাথে কোনো conflict নেই
    ...
    # run_polling() কে directly synchronously call করো
    # asyncio.run() বা await দেওয়া যাবে না — PTB নিজেই loop manage করে
    # close_loop=False দিলে shutdown-এ loop.close() conflict হয় না
    app.run_polling(
