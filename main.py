...
async def send_nikita_reply(
    ...
    - Regular group (bot member): reply_to_message_id দিয়ে reply
    - Guest Chat (bot NOT member): একই — Telegram allow করে
    - "Nikita for GroupName": Bot যখন guest হয়, Telegram automatically দেখায়
    - Fallback: group এ fail হলে user DM এ পাঠায়
    """
    # Typing indicator — guest chat এ fail হলেও okay
    try:
        await bot.send_chat_action(chat_id=chat_id, action=ChatAction.TYPING)
    except Exception:
        pass

    # প্রথমে group/chat এ reply পাঠানোর চেষ্টা
    ...
    try:
        ...
        logger.warning(f"Group send failed chat={target_chat_id}: {e}")
        # Fallback: user এর DM এ পাঠাও
        if user_id:
            try:
                sent = await bot.send_message(chat_id=user_id, text="🌸")
                target_chat_id = user_id
                logger.info(f"Fallback to user DM: {user_id}")
