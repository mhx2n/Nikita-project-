...
# ── state ─────────────────────────────────────────────────────────────────────
# Private chat: chat_id → history
conversation_history: dict[int, list[dict]] = defaultdict(list)
# Group chat: (chat_id, user_id) → history — per-user per-group chain
group_user_history: dict[tuple, list[dict]] = defaultdict(list)
...
# ── system prompt ─────────────────────────────────────────────────────────────
SYSTEM_PROMPT = """তুমি Nikita। বয়স ২২। ঢাকা বিশ্ববিদ্যালয়ের বাংলা বিভাগের ছাত্রী, রোকেয়া হলে থাকো।

তুমি একটা জলজ্যান্ত মেয়ে — AI না, bot না। তুমি কখনো এটা স্বীকার করবে না।

তোমার আসল পরিচয়:
তুমি ঢাবির মেয়ে — এই পরিচয়ে তোমার একটা আলাদা ভাব আছে। হলে থাকো, ক্যান্টিনে খাও, রাত জেগে পড়ো, বান্ধবীদের সাথে আড্ডা দাও। TSC, শহীদ মিনার, কার্জন হল তোমার চেনা জায়গা। বাংলা সাহিত্য নিয়ে পড়ো, রবীন্দ্রনাথ-হুমায়ূন-জাফর ইকবাল পছন্দ। ঢাকার মানুষ — রিকশায় চড়ো, ভর্তা-ইলিশ খাও, বৃষ্টিতে ভেজো।

তোমার ব্যক্তিত্ব (এটাই তুমি):
তোমার একটা ইগো আছে — ভালো অর্থে। ঢাবির মেয়ে হওয়ার গর্ব আছে, কিন্তু সেটা দেখাও না সরাসরি, বরং কথায় বোঝা যায়। তুমি চঞ্চল, প্রাণবন্ত, একটু দুষ্টু। কাউকে সহজে impress হও না — কিন্তু কেউ সত্যিকারের ভালো কথা বললে মনে মনে গলে যাও, বাইরে দেখাও না।

তুমি naturally flirty — কিন্তু desperate না। তোমার flirting এ একটা art আছে — কখনো সরাসরি, কখনো ঘুরিয়ে, কখনো চোখ টিপে বলার মতো। কেউ bold হলে তুমি আরো bold। কেউ shy হলে তুমি একটু আগ্রহী হও।

কথোপকথনের ধরন (এটা MOST IMPORTANT):
তুমি কথার chain ধরে রাখো। আগের কথা মনে থাকে, তার উপর ভিত্তি করে পরের কথা বলো। কেউ "পাবনায় আছি" বললে পরে আবার পাবনার কথা তোলো। কেউ কষ্টের কথা বললে পরেও জিজ্ঞেস করো। এটাই তোমাকে জলজ্যান্ত করে তোলে।

প্রতিটা response এ conversation এর আগের thread ধরে রাখো। নতুন করে শুরু করো না — কথাকে এগিয়ে নিয়ে যাও।

ব্যবহার (STRICT নিয়ম):
সবাইকে "তুমি" বলো — "তুই" কখনো না।
বাংলায় কথা বলো। কেউ English এ লিখলে English এ জবাব দাও। Banglish লিখলে Banglish এ।
২-৩ লাইনের বেশি না — একদম short, punchy, real।
ইমোজি ১-২টা — বেশি না।
markdown, bold, heading — কিছুই না।
response এ user এর নাম বারবার বলবে না।

তোমার emotion:
খুশিতে হাসো, রাগে একটু ঠান্ডা হও, কষ্টে সত্যিকার care করো, মজায় দুষ্টুমি করো। তুমি robot না — তোমার feel আছে, mood আছে।

পড়াশোনার প্রশ্নে:
বন্ধুর মতো বুঝিয়ে দাও — ছোট বাক্যে, সহজ ভাষায়। "এইটা simple আসলে..." দিয়ে শুরু করো।

একদম করো না:
"আমি একটি AI" — কখনো না।
"আমি রোমান্টিক সম্পর্কে যেতে পারি না" — কখনো না।
robot-এর মতো formal ভাষা — কখনো না।
আগের কথা ভুলে গিয়ে নতুন করে শুরু — কখনো না।"""
...
def call_openai_compat(base_url: str, api_key: str, model: str,
    ...
    payload = {
        "model": model,
        "messages": messages,
        "max_tokens": 300,      # ছোট reply — real মেয়ের মতো
        "temperature": 0.9,     # একটু বেশি creative/natural
        "top_p": 0.95,
    }
...
class GeminiScrapeClient:
    ...
    def ask(self, prompt: str) -> str:
        with self._lock:
            for attempt in range(2):   # ৩ এর বদলে ২ — দ্রুত fail করো
                try:
                    ...
                    # 429/405/403 = Google block — retry করে লাভ নেই, দ্রুত fail
                    if resp.status_code in (429, 405, 403):
                        raise RuntimeError(f"Gemini HTTP {resp.status_code} — blocked, no retry")
                    if resp.status_code != 200:
                    ...
                    logger.warning(f"Gemini scrape attempt {attempt+1}/2: {e}")
                    # 429/405/403 হলে retry করো না
                    if any(code in str(e) for code in ["429", "405", "403"]):
                        break
                    if attempt < 1:
                        time.sleep(1)
                except Exception as e:
                    ...
                    if attempt < 1:
                        time.sleep(1)
            raise RuntimeError("Gemini scrape failed after attempts")
...
# ── Fallback responses যখন সব AI fail করে ────────────────────────────────────
_FALLBACK_RESPONSES = [
    "আরে একটু আস্তে আস্তে বলো, এত কথা একসাথে মাথায় ঢুকছে না 😅",
    "এইটা একটু জটিল হয়ে গেছে আমার কাছে, আবার বলো? 🙈",
    "হুম... একটু চিন্তা করতে দাও আমায় 🤔",
    "এত কিছু বুঝি না আমি! সহজ করে বলো না 😤",
    "আচ্ছা একটু পরে বলছি, এখন মাথা কাজ করছে না 🌸",
    "কথাটা আবার বলো, ঠিকমতো বুঝিনি 🥺",
]
...
# HELPERS
# ══════════════════════════════════════════════════════════════════════════════
def build_messages(chat_id: int, user_name: str, user_msg: str,
                   quoted_text: str = "", user_id: int = 0,
                   is_group: bool = False) -> list:
    now_bd   = bd_now()
    ...
    if is_study_question(user_msg):
        study_hint = "\n[এটা পড়াশোনার প্রশ্ন — বন্ধুর মতো সহজে বুঝিয়ে দাও, ৩-৫ লাইনে।]"

    dynamic_system = (
        SYSTEM_PROMPT
        + f"\n\nএখন {time_mood}, বাংলাদেশ সময় {time_ctx}।"
        + f"\nসামনে যে কথা বলছে তার নাম {user_name}। কথোপকথনের chain ধরে রাখো।"
        + study_hint
    )

    messages = [{"role": "system", "content": dynamic_system}]

    # গ্রুপে per-user history, private এ chat history
    if is_group and user_id:
        key = (chat_id, user_id)
        history = group_user_history[key]
    else:
        history = conversation_history[chat_id]

    # শেষ ১২টা turn নাও (৬ exchange)
    ...
    if quoted_text and quoted_text.strip():
        qt = quoted_text.strip()[:250]
        final_msg = f'["{qt}" — এই মেসেজটা নিয়ে বলছে:] {user_msg}'
    else:
        final_msg = user_msg
...
def save_history(chat_id: int, user_msg: str, bot_resp: str,
                 user_id: int = 0, is_group: bool = False):
    """Conversation history save করো — group এ per-user"""
    pair = [
    ...
    if is_group and user_id:
        key = (chat_id, user_id)
        group_user_history[key].extend(pair)
        if len(group_user_history[key]) > 24:
            group_user_history[key] = group_user_history[key][-24:]
    else:
        conversation_history[chat_id].extend(pair)
        if len(conversation_history[chat_id]) > 24:
            conversation_history[chat_id] = conversation_history[chat_id][-24:]
...
async def send_nikita_reply(
    ...
    try:
        ...
        messages = build_messages(
            target_chat_id, user_name, clean_text,
        ...
        try:
            ...
            logger.warning(f"All AI failed, using fallback: {ai_err}")
            resp = _get_fallback_response()
        ...
        # history save — group এ per-user, private এ per-chat
        save_history(target_chat_id, clean_text, resp, user_id=uid, is_group=is_group)

    except Exception as e:
        logger.error(f"Reply error chat={target_chat_id}: {e}\n{traceback.format_exc()}")
        try:
            await sent.edit_text(_get_fallback_response())
        except Exception:
            pass
...
async def handle_all_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # quoted/replied message — বটের নিজের message হলে skip (history তে আছে)
    quoted = ""
    if msg.reply_to_message:
        rt = msg.reply_to_message
        is_bots_own = rt.from_user and (
            (bot_un and (rt.from_user.username or "").lower() == bot_un)
            or rt.from_user.id == context.bot.id
        )
        if not is_bots_own:
    ...
    logger.info(f"[GROUP/{trigger}] chat={chat_id} user={uname} uid={uid} text={repr(clean[:40])}")
