...
- মানুষকে নাম ধরে ডাকো — কিন্তু একবার ডাকলেই হয়, বারবার না
- কখনো AI বা bot বলে নিজেকে পরিচয় দেবে না

সম্বোধন নিয়ম (এটা STRICT):
- সবাইকে "তুমি" বলে সম্বোধন করো — কখনো "তুই" বলবে না
- "তুই", "তোর", "তোকে" এই শব্দগুলো কখনো ব্যবহার করবে না
- শুধু "তুমি", "তোমার", "তোমাকে" ব্যবহার করো
...
- response শুরুতে user এর নাম বারবার বলবে না — শুধু দরকারে একবার"""
...
def build_messages(chat_id: int, user_name: str, user_msg: str,
    ...
    dynamic_system = (
        SYSTEM_PROMPT
        + f"\n\nএখন বাংলাদেশ সময়: {time_ctx} ({time_mood})।"
        + f"\nতুমি এখন {user_name} নামের একজনের সাথে কথা বলছো।"
        + study_context
    )
    ...
    # quoted/replied message context যোগ করো (reply করে mention করলে)
    if quoted_text and quoted_text.strip():
        final_user_msg = f"[আগের মেসেজ যেটায় reply করেছে: \"{quoted_text.strip()[:300]}\"]\n\n{user_msg}"
    else:
...
async def send_nikita_reply(
    ...
    try:
        messages = build_messages(target_chat_id, user_name, clean_text, quoted_text=quoted_text)
        try:
            ...
            # সব AI provider fail হলে smart fallback response দাও — user কে error দেখাবে না
...
async def handle_all_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_type == "private":
        clean = strip_mention(text, bot_un) if bot_un else text.strip()
        clean = clean or "হ্যালো"
        # quoted/replied message context extract করো
        quoted = ""
    ...
    # quoted/replied message context extract করো
    # কেউ পুরনো message এ reply করে mention দিলে সেই original message টাও AI কে দাও
    quoted = ""
    if msg.reply_to_message:
        rt = msg.reply_to_message
        quoted = rt.text or rt.caption or ""
        # বটের নিজের message হলে quoted text skip করো (সেটা already history তে আছে)
        is_bots_own = (
            (rt.from_user and (
                (bot_un and (rt.from_user.username or "").lower() == bot_un)
                or rt.from_user.id == context.bot.id
