...
# ══════════════════════════════════════════════════════════════════════════════
_guest_poll_offset: int = 0

async def raw_guest_poll_loop():
    """
    PTB এর সমান্তরালে চলে।
    শুধু guest_message update গুলো pick করে handle করে।
    Regular update গুলো PTB নিজেই নেয় — এখানে শুধু guest_message process হয়।
    """
    global _guest_poll_offset
    ALLOWED = [
    ...
    logger.info("[RAW-POLL] Guest message raw polling loop started")

    while True:
        try:
            params = {
                "offset": _guest_poll_offset,
                "timeout": 30,
                "allowed_updates": ALLOWED,
            }
            r = await asyncio.to_thread(
                lambda: std_requests.post(url, json=params, timeout=40)
            )
            if r.status_code != 200:
                logger.warning(f"[RAW-POLL] HTTP {r.status_code}: {r.text[:200]}")
            ...
            for upd in updates:
                ...
                _guest_poll_offset = upd_id + 1

                # শুধু guest_message handle করো — বাকি সব PTB নেয়
                if "guest_message" not in upd:
                    continue
                ...
                chat      = guest_msg.get("chat") or {}
                chat_id   = chat.get("id", 0)
                bot_un    = BOT_USERNAME.lstrip("@").lower()

                clean = strip_mention(text, bot_un) if bot_un else text.strip()
                clean = clean or "হ্যালো"

                logger.info(f"[RAW-POLL/GUEST] query_id={query_id} chat={chat_id} user={uname} text={repr(clean[:40])}")

                if query_id:
                    asyncio.create_task(
                        _process_guest_query(query_id, chat_id, uname, clean, uid)
                    )

        except asyncio.CancelledError:
            logger.info("[RAW-POLL] Loop cancelled, stopping.")
            break
        except Exception as e:
            logger.error(f"[RAW-POLL] Error: {e}\n{traceback.format_exc()}")
            await asyncio.sleep(5)
...
async def _start_raw_guest_loop(app):
    """PTB Application init হওয়ার পর raw guest poll loop শুরু করো"""
    asyncio.create_task(raw_guest_poll_loop())
    logger.info("[INIT] raw_guest_poll_loop task created ✅")
...
def main():
    ...
    # getUpdates কে guest_message সহ subscribe করাও
    try:
        r = _r.post(
            f"https://api.telegram.org/bot{BOT_TOKEN}/getUpdates",
            json={"allowed_updates": STARTUP_ALLOWED, "offset": -1, "timeout": 1},
            timeout=15
        )
        logger.info(f"getUpdates subscription set: {r.status_code}")
    except Exception as e:
        logger.warning(f"getUpdates subscription failed: {e}")
