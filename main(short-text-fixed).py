...
- সর্বোচ্চ ৩-৪ লাইনে উত্তর দাও। কখনো বড় paragraph লিখবে না। একদম রিয়েল মেয়ের মতো — ছোট, ঝরঝরে, punchy।
...
- long lecture দেবে না — সর্বোচ্চ ৩-৪ লাইন, এর বেশি কখনো না"""
...
async def send_nikita_reply(
    ...
    source_chat_id: int = None,   # guest chat: যে chat থেকে mention এসেছে
    user_id: int = None,           # fallback DM এর জন্য user id
):
    """
    source_chat_id — Guest Chat Mode:
      বট যে chat এ member নয় সেখান থেকে mention এলে,
      source_chat_id = সেই chat এর id (mention এসেছে যেখান থেকে)।
      reply_parameters এ chat_id দিলে "Nikita for Username" দেখায় ও
      cross-chat reply কাজ করে।
    user_id — bot যদি সেই chat এ reply দিতে না পারে,
      তাহলে user এর DM এ fallback করবে।
    """
    from telegram import ReplyParameters

    # Guest chat এ typing action পাঠানো যায় না (bot member নয়), skip করি
    try:
        await bot.send_chat_action(chat_id=chat_id, action=ChatAction.TYPING)
    except Exception:
        pass

    # reply_parameters build — cross-chat guest reply support
    rp = None
    if reply_to_message_id:
        rp_kwargs = {"message_id": reply_to_message_id}
        # Guest Chat: source chat থেকে mention এলে, সেই chat এর message id দিতে হবে
        if source_chat_id and source_chat_id != chat_id:
            rp_kwargs["chat_id"] = source_chat_id
        rp = ReplyParameters(**rp_kwargs)

    # প্রথমে original chat এ পাঠানোর চেষ্টা করো
    ...
    try:
        ...
        logger.warning(f"Group send failed (possibly guest mode) chat={target_chat_id}: {e}")
        # Fallback: user এর DM এ পাঠাও (bot member নয় এই group এ)
        if user_id:
            try:
                sent = await bot.send_message(
                    chat_id=user_id,
                ...
                logger.info(f"Guest mode fallback: sending to user DM {user_id}")
...
async def handle_all_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Guest Chat Mode Support ───────────────────────────────────────────────
    # Telegram May 2026: বট member না হলেও mention করলে update আসে।
    # কিন্তু সেই chat এ directly message পাঠাতে পারে না।
    # Solution: bot এর নিজের chat (user এর DM) এ পাঠাতে হবে,
    # অথবা reply_parameters এ source chat_id দিয়ে cross-chat reply করতে হবে।
    #
    # Guest mode detect: bot কি এই chat এ member?
    # প্রথমে try করি সরাসরি; fail হলে user এর DM এ পাঠাই।
    # ─────────────────────────────────────────────────────────────────────────
    ...
    await send_nikita_reply(
        ...
        clean_text=clean,
        bot=context.bot,
        source_chat_id=chat_id,   # cross-chat reply এর জন্য
        user_id=user.id if user else None,  # guest mode DM fallback
    )
